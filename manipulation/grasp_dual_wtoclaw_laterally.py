...
# Code for grasping a box with both hands without the claw and laterally. These are the parameters that I am selecting:
# -grasp
# -dual-arm symmetric
# -without claw
# -Laterally
...
class DualDaganaReach(Node):
    ...
    def __init__(self):
        ...
        self.box_width = 0.38
        self.phase1_clearance = 0.05       # in fase 1 stiamo più larghi della scatola di 2 cm per lato
        self.phase3_extra_squeeze = 0.05   # in fase 3 stringiamo ancora di 0.5 cm per lato
        self.lift_z_phase3 = 0.2           # alzata finale
    ...
    # =========================================================
    # CALCOLO OFFSETS AUTOMATICI DELLE 3 FASI
    # =========================================================
    def compute_all_phase_offsets(self):
        """
        Calcola:
        - PHASE 1: allineamento x e z, approccio su y con apertura > box_width
        ...
        # -----------------------------------------------------
        # Target PHASE 1
        # End-effector leggermente più larghi della scatola
        # -----------------------------------------------------
        ...
        phase1_dx1 = target1_x_p1 - dag1_x
        phase1_dy1 = target1_y_p1 - dag1_y
        phase1_dz1 = target1_z_p1 - dag1_z

        phase1_dx2 = target2_x_p1 - dag2_x
        phase1_dy2 = target2_y_p1 - dag2_y
        phase1_dz2 = target2_z_p1 - dag2_z
        ...
        self.get_logger().info(
            f'PHASE 1 dag1: dx={phase1_dx1:.6f}, dy={phase1_dy1:.6f}, dz={phase1_dz1:.6f}'
        )
        self.get_logger().info(
            f'PHASE 1 dag2: dx={phase1_dx2:.6f}, dy={phase1_dy2:.6f}, dz={phase1_dz2:.6f}'
        )
        self.get_logger().info(
        ...
        return {
            'phase1': (
                phase1_dx1, phase1_dy1, phase1_dz1,
                phase1_dx2, phase1_dy2, phase1_dz2
            ),
            'phase2': (
    ...
    def run_phase(self, phase_name: str, dx1, dy1, dz1, dx2, dy2, dz2) -> None:
        goal1 = self._make_goal(dx1, dy1, dz1)
        goal2 = self._make_goal(dx2, dy2, dz2)
    ...
    def execute(self):
        ...
        # =====================================================
        self.get_logger().info(
            'Rilettura posizioni dopo INIT_ROT per calcolare le 3 fasi...'
        )
        self.read_all_positions()
        ...
        # PHASE 1
        # =====================================================
        self.run_phase("PHASE 1", *phases['phase1'])

        # =====================================================
        # PHASE 2
        # =====================================================
        self.run_phase("PHASE 2", *phases['phase2'])

        # =====================================================
        # PHASE 3
        # =====================================================
        self.run_phase("PHASE 3", *phases['phase3'])
