...
def build_step_lookup(sequential_plan: Any) -> dict[int, dict[str, Any]]:
    if not isinstance(sequential_plan, list):
        raise ValueError("vlm_planning output must be a JSON array of steps.")

    step_lookup: dict[int, dict[str, Any]] = {}

    for idx, step in enumerate(sequential_plan):
        if not isinstance(step, dict):
            raise ValueError(f"Planning step at index {idx} is not a JSON object.")
        ...
        if not isinstance(step_id, int):
            raise ValueError(f"Planning step at index {idx} has invalid or missing 'Step_id'.")
        if step_id in step_lookup:
            raise ValueError(f"Duplicate planning Step_id found: {step_id}")
        if not isinstance(target, str) or not target.strip():
            raise ValueError(f"Planning step {step_id} has invalid or missing 'Target'.")
        if not isinstance(action_concept, str) or not action_concept.strip():
            raise ValueError(f"Planning step {step_id} has invalid or missing 'Action concept'.")
...
def build_execution_plan(
    ...
    step_lookup = build_step_lookup(sequential_plan)
    ...
    for stage in stages:
        ...
        if not isinstance(step_ids, list) or not step_ids:
            raise ValueError(
                f"Stage {stage_id} does not contain a valid 'Step_id' list. "
                "Update extract_stages() so it preserves Step_id from simultaneous_actions."
            )
        if not all(isinstance(step_id, int) for step_id in step_ids):
            raise ValueError(f"Stage {stage_id} has non-integer values inside 'Step_id'.")

        executable_steps: list[dict[str, Any]] = []
        for step_id in step_ids:
            if step_id not in step_lookup:
                raise ValueError(
                    f"Stage {stage_id} references Step_id={step_id}, "
                    "but that step is not present in vlm_planning output."
                )
            executable_steps.append(step_lookup[step_id])

        if len(executable_steps) != 1:
            raise ValueError(
                f"Stage {stage_id} contains {len(executable_steps)} planning steps. "
                "Automatic live execution currently supports exactly one robot action per stage. "
                "Split simultaneous actions into separate stages or extend the local host executor."
            )

        executable_step = executable_steps[0]

        execution_plan.append(
            {
                "stage_id": stage_id,
                "step_ids": step_ids,
                "action": executable_step["action"],
                "object": executable_step["object"],
                "action_concept": executable_step["action_concept"],
                "target": executable_step["target"],
                "manipulation_mode": executable_step.get("manipulation_mode"),
                "contact_mode": executable_step.get("contact_mode"),
                "object_contact_side": executable_step.get("object_contact_side"),
                "precondition": stage["Precondition"],
                "postcondition": stage["Postcondition"],
                "raw_stage": stage,
                "raw_step": executable_step["raw_step"],
            }
        )
...
def build_execution_plan_by_stage(
    execution_plan: list[dict[str, Any]],
) -> dict[int, dict[str, Any]]:
    return {item["stage_id"]: item for item in execution_plan}


def print_execution_plan(execution_plan: list[dict[str, Any]]) -> None:
    print("\n======================================================")
    print("FINAL EXECUTABLE PLAN FROM simultaneous_actions")
    print("======================================================")

    for item in execution_plan:
        print(f"Stage {item['stage_id']}")
        print(f"  Step IDs:          {item['step_ids']}")
        print(f"  Action:            {item['action']}")
        print(f"  Object:            {item['object']}")
        print(f"  Action concept:    {item['action_concept']}")
        if item.get("manipulation_mode") is not None:
            print(f"  Manipulation mode: {item['manipulation_mode']}")
        if item.get("contact_mode") is not None:
            print(f"  Contact mode:      {item['contact_mode']}")
        if item.get("object_contact_side") is not None:
            print(f"  Contact side:      {item['object_contact_side']}")
        print(f"  PRE:               {item['precondition']}")
        print(f"  POST:              {item['postcondition']}")
        print("------------------------------------------------------")
...
def ask_execution_plan_approval() -> bool:
    print()
    print("Approve this executable plan before robot execution? [y/N]")
    print("Type your answer and press Enter.", flush=True)
    answer = input("> ").strip()
    return answer in {
        "y", "Y",
        "yes", "Yes", "YES",
...
def build_action_request_for_stage(
    execution_plan_by_stage: dict[int, dict[str, Any]],
    stage_id: int,
) -> dict[str, Any]:
    if stage_id not in execution_plan_by_stage:
        raise KeyError(f"No executable action found for stage {stage_id}.")

    item = execution_plan_by_stage[stage_id]
    return {
        "stage_id": item["stage_id"],
        "step_ids": item["step_ids"],
        "action": item["action"],
        "object": item["object"],
        "action_concept": item["action_concept"],
        "target": item["target"],
        "manipulation_mode": item.get("manipulation_mode"),
        "contact_mode": item.get("contact_mode"),
        "object_contact_side": item.get("object_contact_side"),
    }
...
def print_action_request(action_request: dict[str, Any]) -> None:
    print(
        "[ACTION_REQUEST] "
        + json.dumps(action_request, ensure_ascii=False, separators=(",", ":")),
        flush=True,
    )
...
def main() -> None:
    ...
    try:
        ...
        execution_plan_by_stage = build_execution_plan_by_stage(execution_plan)
        cycle_record["execution_plan"] = execution_plan

        print_execution_plan(execution_plan)

        if not ask_execution_plan_approval():
            print("\n[PLAN] Executable plan was rejected by the user. Stopping before robot execution.")
            cycle_record["outcome"] = "plan_rejected_by_user"
            all_stages_succeeded = False
        else:
            print("\n[PLAN] Executable plan approved. Starting automatic robot execution.")
            ...
            for stage in stages:
                ...
                action_request = build_action_request_for_stage(
                    execution_plan_by_stage=execution_plan_by_stage,
                    stage_id=stage_id,
                )
                stage_record["action_request"] = action_request

                print(f"\n[LOOP] Stage {stage_id} ACTION")
                print(
                    "[LOOP] Requesting automatic robot action from the local host executor."
                )
