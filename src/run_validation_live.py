#### V2: VERSION THAT CONSIDERS STEP.ID SO TO CONNECT BOTH THE SIMULTANEOUS ACTIONS AND THE VLM PLANNING MODULES -> THIS IS DONE SO THAT WE CAN AVOID ASKING BY TERMINAL THE ACTION AND THE TARGET OBJECT
...
def extract_stages(compact_parallel_plan: Any) -> list[dict[str, Any]]:
    ...
    for idx, stage in enumerate(compact_parallel_plan):
        ...
        # Step_id links each simultaneous_actions stage back to one or more
        # vlm_planning steps. Keep it here so live2 can recover the executable
        # action/object pair for each stage.
        if isinstance(step_id, int):
            step_ids = [step_id]
        elif isinstance(step_id, list) and step_id and all(isinstance(item, int) for item in step_id):
            step_ids = step_id
        else:
            raise ValueError(
                f"Stage {stage_id} has invalid or missing 'Step_id'. "
                "Expected an integer or a non-empty list of integers."
            )
...
#### V1 -> IDENTICAL TO RUN_VALIDATION_IMAGE.PY EXCEPT FOR ONE LINE (PARAM RELATED TO NAMING CONVENTION OF THE SCENE ENRICHMENT)
