...
def finalize_cycle_timing(cycle_record: dict[str, Any]) -> None:
    ...
    for stage in cycle_record.get("stages", []):
        ...
        pre_t = safe_float(timing.get("pre_validation"))
        post_t = safe_float(timing.get("post_validation"))
        deploy_t = safe_float(timing.get("deploy"))
        total_t = safe_float(timing.get("total"))
...
def execute_scene_description_step(
    ...
    print(f"[DEBUG] Image verified immediately before Azure call: {image_path}")
    print(f"[DEBUG] Image size bytes: {Path(image_path).stat().st_size}")
...
def execute_validator_step(
    ...
    print(f"[DEBUG] Validator image verified before Azure call: {image_path}")
    print(f"[DEBUG] Validator image size bytes: {Path(image_path).stat().st_size}")
...
def main() -> None:
    ...
    while not task_completed:
        ...
        try:
            ...
            cycle_record["timing"]["scene_description"] = scene_description_artifact.get("execution_time_seconds")
            ...
            cycle_record["timing"]["scene_enrichment"] = scene_description_full_artifact.get("execution_time_seconds")
            ...
            cycle_record["timing"]["planning"] = sequential_plan_artifact.get("execution_time_seconds")
            ...
            cycle_record["timing"]["simultaneous"] = simultaneous_actions_artifact.get("execution_time_seconds")
            ...
            for stage in stages:
                ...
                stage_record["timing"]["pre_validation"] = pre_artifact.get("execution_time_seconds")
                ...
                stage_record["timing"]["post_validation"] = post_artifact.get("execution_time_seconds")
                stage_record["timing"]["total"] = time.perf_counter() - stage_start_time
                cycle_record["stages"].append(stage_record)
...
# ### FROM DOCKER -> WORKING VERSION WITHOUT TIMESTAMP FOR MANIPULATION PART 
...
# #     # Garantisce che il Python ROS di Jazzy veda i suoi site-packages
# #     ros_pythonpath = "/opt/ros/jazzy/lib/python3.12/site-packages"
# #     existing_pythonpath = env.get("PYTHONPATH", "")
