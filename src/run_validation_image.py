...
#         default=None,
#         help=(
#             "Optional explicit initial image path. If omitted, uses scenario.json image. "
#             "In both cases, the image must belong to image_data/<scenario>."
#         ),
#     )
...
#             f"Scenario image directory not found: {scenario_dir}"
#         )
#     if not scenario_dir.is_dir():
#         raise ValueError(
#             f"Scenario image path is not a directory: {scenario_dir}"
#         )
...
# def is_path_within_directory(candidate_path: Path, root_dir: Path) -> bool:
#     try:
...
# def validate_image_path_for_scenario(image_path: str | Path, scenario_image_dir: Path) -> str:
...
#             f"Allowed: {sorted(IMAGE_EXTENSIONS)}"
#         )
#     if not is_path_within_directory(path, scenario_image_dir):
#         raise ValueError(
#             f"Image must be inside scenario directory '{scenario_image_dir}', found: {path}"
#         )

#     return str(path)


# def prompt_user_for_image_path(
#     scenario_image_dir: Path,
#     poses_by_image: dict[str, dict[str, list[float]]],
# ) -> str:
#     while True:
#         print("\n[SELECTING NEW IMAGE]")
#         print(f"Scenario directory: {scenario_image_dir}")
#         user_input = input("Insert path: ").strip()

#         if not user_input:
#             print("[ERROR] Empty path. Please insert a valid image path.")
#             continue

#         try:
#             selected_image = validate_image_path_for_scenario(user_input, scenario_image_dir)
#             _ = get_pose_dict_for_image(poses_by_image, selected_image)
#             print(f"[OK] Selected image: {selected_image}")
#             print(f"[OK] Pose found for:   {Path(selected_image).name}")
#             return selected_image
#         except Exception as exc:
...
#         },
#         "image_input": {
#             "mode": "manual_terminal_selection",
#             "scenario_image_dir": str(scenario_image_dir),
#         },
#         "scene_description": {
#             "prompt_version": args.scene_v,
#             "model": args.scene_model,
...
#     initial_image_path = (
#         validate_image_path_for_scenario(args.initial_image_path, scenario_image_dir)
...
#         raise ValueError(
#             "No initial image available. Provide --initial-image-path or set 'image_path_abs' in scenario.json."
#         )

#     initial_image_path = validate_image_path_for_scenario(initial_image_path, scenario_image_dir)

#     _ = get_pose_dict_for_image(poses_by_image, initial_image_path)
...
#     print(f"Top-p:                     {args.top_p}")
#     print(f"Max replans:               {args.max_replans}")
#     print("Image input mode:          manual terminal selection")
#     print("======================================================")
...
#                 # --------------------------------------------------
#                 # MANUAL DEPLOY / MANUAL NEXT IMAGE
#                 # --------------------------------------------------
#                 print(f"\n[LOOP] Stage {stage_id} deploy succeeded.")
#                 print("[LOOP] A new image is now required for post-validation.")
#                 next_image = prompt_user_for_image_path(
#                     scenario_image_dir=scenario_image_dir,
#                     poses_by_image=poses_by_image,
#                 )
