"""
run_fixed_stage_recovery_probe.py — probe tool to test repeat/modify/replan
decisions with a hand-authored stage, bypassing vlm_planning/simultaneous_actions.

The wording of Preconditions/Postconditions is the only thing that varies
between runs of the real pipeline (vlm_planning + simultaneous_actions are not
fully deterministic), which makes the repeat/modify/replan decision hard to
reproduce on demand. This script removes that variable: you supply the stage
and its actions from a JSON file, and only the validator's own visual
judgement on the images you choose as I_pre/I_post remains uncontrolled.

It does not modify or duplicate any existing pipeline file. Every building
block (validators, recovery engine, modification-proposal step) is imported
unchanged from run_validation_loop_fixed_on_demand.py and its sibling modules;
this script only adds the orchestration needed to run a single fixed stage
interactively, attempt after attempt, and print the recovery decision.

Example fixed-stage file:
{
  "stage": {
    "Stage_id": 1,
    "Step_id": [1],
    "Local_goal": "Push box_2 from its front side to free the right side of box_1.",
    "Preconditions": [
      "end_effector_right is grasping nothing.",
      "The front side of box_2 is accessible."
    ],
    "Postconditions": [
      "The right side of box_1 is accessible."
    ]
  },
  "actions": [
    {
...
from src.humanoids.scene_transition_analysis import analyze_scene_transition
...
from src.humanoids.run_validation_loop_fixed_on_demand import (
    ...
    execute_scene_perception_for_state,
...
def main() -> None:
    ...
    print("\n======================================================")
    print("FIXED-STAGE RECOVERY PROBE")
    ...
    for attempt_number in range(1, args.max_attempts + 1):
        ...
        recovery_pre_scene_graph = execute_scene_perception_for_state(
            settings=settings,
            scenario_name=args.scenario,
            ...
            safety_threshold=args.grounding_safety_threshold,
            include_debug_mapping=args.grounding_debug_mapping,
            output_dir=Path(settings.project_root) / "outputs" / "fixed_stage_probe" / args.scenario / loop_timestamp / cycle_name / "pre_recovery",
            purpose=f"pre_recovery_state_stage_{stage_id}",
        )["scene_graph"]
        post_scene_graph = execute_scene_perception_for_state(
            settings=settings,
            scenario_name=args.scenario,
            scenario_data=scenario_data,
            ...
            safety_threshold=args.grounding_safety_threshold,
            include_debug_mapping=args.grounding_debug_mapping,
            output_dir=Path(settings.project_root) / "outputs" / "fixed_stage_probe" / args.scenario / loop_timestamp / cycle_name / "post_recovery",
