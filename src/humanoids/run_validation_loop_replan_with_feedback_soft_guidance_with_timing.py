...
This file is a copy of `run_validation_loop_replan_with_feedback_soft_guidance.py`
with one addition: total execution timing. It measures wall-clock time from
the start of `main()` to the end, tracks time spent blocked on the two
`--interactive` prompts (`prompt_for_post_image` and the "press Enter to
continue" prompt) separately, and prints both the total elapsed time and the
net execution time (elapsed minus interactive wait) as part of the final
summary block. Everything else is unchanged. """
...
def classify_model_family(args: argparse.Namespace) -> str:
    """
    Bucket a run as "gemini" or "gpt" for terminal-log organization only.

    "gemini" = every core module (scene/plan/sim/validator) uses
    gemini-robotics-er-1.6-preview. Anything else -- including the
    gpt-5.2-everywhere-except-o3-for-planning configuration -- is bucketed
    as "gpt", since the planner model is what distinguishes the two
    configurations actually used for this scenario.
    """
    core_models = {
        args.scene_model,
        args.plan_model,
    ...
    if core_models == {"gemini-robotics-er-1.6-preview"}:
        return "gemini"
    return "gpt"
...
def resolve_terminal_log_path(args: argparse.Namespace) -> Path:
    ...
    return (
        Path("a_data_analysis_final")
        / "terminal_logs"
        / str(args.scenario)
        / classify_model_family(args)
...
def main() -> None:
        ...
        try:
            ...
            for stage_position, stage in enumerate(stages, start=1):
                ...
                if pre_status == "satisfied":
                    ...
                    if args.interactive:
                        _interactive_wait_start = time.perf_counter()
                        post_cursor_override = prompt_for_post_image(frame_paths, frame_cursor)
                        interactive_wait_seconds += time.perf_counter() - _interactive_wait_start
                    else:
                        post_cursor_override = None
                    try:
    ...
    execution_seconds = total_elapsed_seconds - interactive_wait_seconds

    print("\n======================================================")
    print("EXECUTION TIME (excludes time spent waiting on --interactive prompts)")
    print(f"Total wall-clock time:  {total_elapsed_seconds:.2f}s ({total_elapsed_seconds / 60:.2f} min)")
    print(f"Interactive wait time:  {interactive_wait_seconds:.2f}s ({interactive_wait_seconds / 60:.2f} min)")
    print(f"Execution time (net):   {execution_seconds:.2f}s ({execution_seconds / 60:.2f} min)")
    print("======================================================")
