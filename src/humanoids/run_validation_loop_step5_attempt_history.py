...
def close_attempt(
    ...
    """
    Close an attempt exactly once and assign its terminal outcome.
    ...
    if status not in status_to_outcome:
        raise ValueError(
            "A closed attempt must use closed_not_executed, "
            "closed_success, or closed_failure."
        )

    if attempt.get("closed_at") is not None:
        raise ValueError(
            f"Attempt {attempt.get('attempt_id')} is already closed."
        )

    if status == "closed_failure" and failure_report is None:
        raise ValueError(
            "closed_failure requires a failure_report."
        )

    if status == "closed_success" and failure_report is not None:
        raise ValueError(
            "closed_success cannot contain a failure_report."
        )

    attempt["status"] = status
    attempt["outcome"] = status_to_outcome[status]
    attempt["closed_at"] = datetime.now().isoformat()
...
def validate_validator_response(
    ...
    if overall_status not in VALIDATOR_STATUSES:
        raise ValueError(
            "Validator field 'overall_status' must be satisfied, violated, or uncertain."
        )
    if not isinstance(results, list):
        raise ValueError("Validator field 'results' must be a list.")
    if len(results) != len(expected_preconditions):
        raise ValueError(
            "Validator returned a different number of results than input preconditions."
        )

    for idx, (item, expected_condition) in enumerate(zip(results, expected_preconditions)):
        if not isinstance(item, dict):
        ...
        if item.get("status") not in VALIDATOR_STATUSES:
            raise ValueError(f"Validator result at index {idx} has invalid status.")
...
def execute_validator_step(
    ...
    if not parse_ok:
        raise ValueError(
            f"[validator:pre_{stage_id}] Model response could not be parsed as valid JSON.\n\n"
            f"Raw response:\n{result['raw_response']}"
        )

    validate_validator_response(parsed_response, preconditions)
    ...
    run_info = {
        "module": "validator",
        "execution_mode": "precondition_batch_validation",
        "scenario_name": scenario_name,
    ...
    print(f"[OK][validator:pre_{stage_id}] Prompt saved to:        {prompt_path}")
    print(f"[OK][validator:pre_{stage_id}] Parsed output saved to: {parsed_path}")
    print(f"[OK][validator:pre_{stage_id}] Run info saved to:      {run_info_path}")
    print(f"[OK][validator:pre_{stage_id}] Execution time:         {result['execution_time_seconds']:.3f}s")
...
def execute_postcondition_validator_step(
    ...
    validate_validator_response(parsed_response, postconditions)
    parsed_response["overall_status"] = compute_overall_status(
    ...
    while not task_completed:
        ...
        try:
            ...
            for stage_position, stage in enumerate(stages, start=1):
                ...
                # One attempt per scheduled stage. Recovery retries can later
                # increase this index for the same stage.
                attempt_idx = 1
                ...
                if pre_status == "satisfied":
                    ...
                    # The normal execution step advances the physical state to
                    # I_post. That frame is reused as I_pre for the next stage.
                    attempt_record["post"]["scene_description_full_path"] = None
                    ...
                    if post_status == "satisfied":
                        ...
                        violated_post_results = [
                            item
                            for item in post_response["results"]
                        ...
                        failure_report = {
                            ...
                            "execution_completed": bool(
                                attempt_record["execution"]["completed"]
                            ),
                            "pre_image_path": attempt_record["pre"]["image_path"],
                            "post_image_path": post_image,
                            "violated_conditions": violated_post_results,
                        ...
                        uncertain_post_results = [
                            item
                            for item in post_response["results"]
                            if item["status"] == "uncertain"
                        ]
                        attempt_record["post"]["evidence_rounds"].append(
                            {
                                "round": 1,
                                "pre_image_path": attempt_record["pre"]["image_path"],
                                "post_image_path": post_image,
                                "validation": post_response,
                                "uncertain_conditions": uncertain_post_results,
                                "timestamp": datetime.now().isoformat(),
                            }
                        )
                        stage_record["attempt_outcome"] = None
                        stage_record["attempt_history_event_id"] = None
                        all_stages_succeeded = False
                    ...
                    violated_results = [
                        item
                        for item in pre_response["results"]
                        if item["status"] == "violated"
                    ]
                    failure_report = {
                        ...
                        "image_path": str(Path(current_image).resolve()),
                        "violated_conditions": violated_results,
                        "created_at": datetime.now().isoformat(),
                    }
                    close_attempt(
                        attempt=attempt_record,
                    ...
                    attempt_record["pre"]["evidence_rounds"].append(
                        {
                            "round": 1,
                            "image_path": str(Path(current_image).resolve()),
                            "validation": pre_response,
                            "uncertain_conditions": uncertain_results,
                            "timestamp": datetime.now().isoformat(),
                        }
                    )
                    set_attempt_status(
                    ...
                    stage_record["attempt_history_event_id"] = None
                    cycle_record["outcome"] = (
                        f"preconditions_uncertain_stage_{stage_id}"
                    )
                    all_stages_succeeded = False
                    print(
                        f"[LOOP] Attempt {attempt_record['attempt_id']} "
                        "remains open and requires additional evidence."
                    )
                    print(
                        f"[LOOP] Stopping stage sequence at Stage {stage_id}: "
                        "PRE status is uncertain."
                    )
                    break
            ...
            if all_stages_succeeded and completed_stage_count == len(stages):
                cycle_record["outcome"] = "all_stages_completed_successfully"
                full_summary["task_completed"] = True
                task_completed = True
                print(
                    f"\n[LOOP] All {len(stages)} scheduled stages completed "
                    "successfully."
                )
            else:
                ...
                task_completed = True
                print(
                    f"\n[LOOP] Stage sequence stopped after "
                    f"{completed_stage_count}/{len(stages)} successful stages."
                )
...
#             # Current implementation stops at PRE-condition validation.
#             # Therefore only the current attempt (the first scheduled stage) is checked.
#             stage = stages[0]
#             stage_id = stage["Stage_id"]
#             stage_name = make_stage_name(stage_id)
#             planned_stage_context = build_planned_stage_context(stage)
...
#             # For now there is one attempt for the first scheduled stage.
#             # Later this index will increase when recovery retries the same stage.
#             attempt_idx = 1
#             attempt_record = open_attempt(
#                 cycle_idx=cycle_idx,
#                 stage=stage,
...
#                 # Do not rerun scene perception after normal execution.
#                 # The latest scene_description_full remains the one generated
#                 # at initialization or after the most recent replanning.
#                 attempt_record["post"]["scene_description_full_path"] = None
...
#             # Development stop point after point 5.
#             # Terminal attempts are now closed and written to global history.
#             # Uncertain attempts remain open and are intentionally excluded
#             # from terminal history until evidence gathering resolves them.
#             task_completed = True
#             full_summary["task_completed"] = (
#                 attempt_record["status"] == "closed_success"
#             )
...
# # ) -> tuple[str, int]:
# #     """
# #     Simulate one stage execution by consuming the next prepared frame.

# #     The returned image is I_post. This function does not validate
# #     postconditions and does not run scene perception on I_post yet.
# #     """
# #     if attempt.get("status") != "preconditions_satisfied":
# #         raise ValueError(
# #             "Offline execution can start only from an attempt whose "
...
# #         raise RuntimeError(
# #             "No post-execution frame is available in --frames-dir for "
...
# #     attempt["execution"]["started"] = True
# #     attempt["execution"]["started_at"] = datetime.now().isoformat()
# #     attempt["execution"]["mode"] = "offline_frame_advance"

# #     post_image_path = str(Path(frame_paths[frame_cursor]).resolve())
# #     frame_cursor += 1
...
# #     return post_image_path, frame_cursor
