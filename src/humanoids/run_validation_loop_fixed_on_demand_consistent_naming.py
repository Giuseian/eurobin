...
In short: `run_validation_loop.py` is an automatic offline validation and replanning loop. It tests the full planning/replanning logic over a sequence of prepared images, without Gazebo, robot execution, or manual image selection during the run.

--- consistent-naming variant ---
This file is an exact copy of run_validation_loop_fixed_on_demand.py, wired to
two naming-consistency fixes instead of the original scene-perception /
scene-transition functions (see scene_perception_consistent_naming.py and
scene_transition_analysis_robust.py for why). The original file is left
untouched; this one exists so both can be run and compared side by side.
The only changes relative to the original are: the two imports below, the
`reference_scene_description` parameter added to `gather_and_revalidate_evidence`,
and passing that reference through at every call site of the two swapped-in
...
from src.humanoids.scene_transition_analysis_robust import (
    analyze_scene_transition_robust as analyze_scene_transition,
)
from src.humanoids.scene_perception_consistent_naming import (
    execute_scene_perception_for_state_consistent_naming,
)
from src.humanoids.validator_retry import call_and_normalize_with_retry
...
def execute_validator_step(
    ...
    def _call_pre_validator_llm() -> dict[str, Any]:
    ...
    def _normalize_pre_validator_response(parsed, llm_result):
        return normalize_validation_result(
            raw_response=parsed,
            ...
            validator_metadata={
                ...
                "deployment_name": llm_result["deployment_name"],
            },
        )

    parsed_response, result = call_and_normalize_with_retry(
        call_llm_fn=_call_pre_validator_llm,
        normalize_fn=_normalize_pre_validator_response,
        label=f"validator:{condition_kind}_{stage_id}",
        max_retries=2,
    )
...
def execute_goal_baseline_validator_step(
    ...
    def _call_goal_baseline_llm() -> dict[str, Any]:
        return call_llm_completion(
            settings=settings,
            model_name=validator_model,
            system_prompt=system_prompt,
    ...
    def _normalize_goal_baseline_response(parsed, llm_result):
        return normalize_validation_result(
            raw_response=parsed,
    ...
    parsed_response, result = call_and_normalize_with_retry(
        call_llm_fn=_call_goal_baseline_llm,
        normalize_fn=_normalize_goal_baseline_response,
        label=f"validator:goal_baseline_{stage_id}",
        max_retries=2,
    )
...
def execute_postcondition_validator_step(
    ...
    def _call_post_validator_llm() -> dict[str, Any]:
        return call_llm_completion(
            settings=settings,
            model_name=validator_model,
            system_prompt=system_prompt,
    ...
    def _normalize_post_validator_response(parsed, llm_result):
        return normalize_validation_result(
            raw_response=parsed,
            expected_conditions=postconditions,
            phase="post",
    ...
    parsed_response, result = call_and_normalize_with_retry(
        call_llm_fn=_call_post_validator_llm,
        normalize_fn=_normalize_post_validator_response,
        label=f"validator:post_{stage_id}",
        max_retries=2,
    )
...
""".strip()
    ...
    def _call_evidence_validator_llm() -> dict[str, Any]:
        return call_llm_completion(
            settings=settings,
            model_name=validator_model,
            system_prompt=system_prompt,
    ...
    def _normalize_evidence_validator_response(parsed, llm_result):
        return normalize_validation_result(
            raw_response=parsed,
    ...
    normalized, result = call_and_normalize_with_retry(
        call_llm_fn=_call_evidence_validator_llm,
        normalize_fn=_normalize_evidence_validator_response,
        label=f"evidence_validator:{phase}:{round_index}",
        max_retries=2,
    )
...
def gather_and_revalidate_evidence(
    *,
    ...
    `reference_scene_description` (consistent-naming variant): the object
    identities already established earlier in this cycle, passed through to
    the scene-perception call so it reuses them instead of inventing new
    names for the same physical objects on every fresh evidence round.
    """
    if phase not in {"pre", "post"}:
        raise ValueError(f"Unsupported evidence phase: {phase!r}")
    ...
    for round_index in range(1, max_evidence_rounds + 1):
        ...
        perception = execute_scene_perception_for_state_consistent_naming(
            settings=settings,
            scenario_name=scenario_name,
            scenario_data=scenario_data,
            image_path=image_path,
...
""".strip()
    ...
    def _call_final_goal_llm() -> dict[str, Any]:
        return call_llm_completion(
            settings=settings,
            model_name=model_name,
    ...
    def _normalize_final_goal_response(parsed, llm_result):
    ...
    parsed_fields, result = call_and_normalize_with_retry(
        call_llm_fn=_call_final_goal_llm,
        normalize_fn=_normalize_final_goal_response,
        label="final_goal_validator",
        max_retries=2,
    )
    status = parsed_fields["status"]
    reason = parsed_fields["reason"]
    unsatisfied = parsed_fields["unsatisfied"]
...
def main() -> None:
    ...
    print("\n======================================================")
    print("VALIDATION LOOP CONFIG (consistent-naming fix)")
            ...
            for stage_position, stage in enumerate(stages, start=1):
                ...
                if pre_status == "satisfied":
                    ...
                    if post_status != "satisfied" and not post_scene_graph:
                        ...
                        post_perception = execute_scene_perception_for_state_consistent_naming(
                            settings=settings,
                            scenario_name=args.scenario,
                            scenario_data=scenario_data,
                    ...
                    if post_status != "satisfied":
                        ...
                        pre_recovery_perception = execute_scene_perception_for_state_consistent_naming(
                            settings=settings,
                            scenario_name=args.scenario,
                            scenario_data=scenario_data,
