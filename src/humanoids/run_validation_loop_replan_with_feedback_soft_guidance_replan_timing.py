""" `run_validation_loop_replan_with_feedback_soft_guidance_replan_timing.py` is a
timing-instrumented copy of `run_validation_loop_replan_with_feedback_soft_guidance.py`
(the hierarchical validator pipeline), built for the Humanoids paper's
Table~\\ref{tab:validator_latency} comparison. Behavior is otherwise identical to the
original; the only additions are:

1. A "recovery_timings" list on the run's full_summary. Each entry times one
   repeat/modify/replan recovery event, from the instant a cycle confirms it has a
   failed attempt (failure_report/interpret_failure/[modification_proposal for
   modify]/plan_recovery_evidence_based/schedule_recovery all happen inside this
   window) until the PRE validation status of the first stage of the cycle that
   resumes it is known again -- i.e. it also covers the resuming cycle's
   scene_description + scene_description_full + PRE validation (repeat/modify,
   which skip vlm_planning/simultaneous_actions) or the same plus vlm_planning +
   simultaneous_actions (replan). Any --interactive "Press Enter" pause between the
   decision and the next cycle is measured and subtracted out.
2. The terminal log and a run_summary.json (full_summary, including
   recovery_timings) are saved under
   `humanoids_results/ablation_study/validator/non_binary/<scenario>/<loop_timestamp>/`,
   using the same run_with_terminal_log() mechanism the original already had --
   only the log's target directory changed.
3. A "RECOVERY LATENCY" report (per-event and aggregated by decision type) is
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "a timestamped .txt file is created under "
            "a_data_analysis_final/terminal_logs/<scenario>/<gemini|gpt>/."
        ),
    )
    parser.add_argument(
        "--no-terminal-log",
...
def load_scene_object_list_from_cycle(
    ...
    if not isinstance(data, (dict, list)):
        raise ValueError(
            "scene_object_list.json must contain a JSON object or array."
        )
    return data
...
def render_validator_prompt(
    ...
    Both placeholders are accepted for backward-compatible templates, but both
    are populated with the static object list. The enriched graph is excluded.
    """
...
def execute_vlm_planning_step(
    ...
    if recovery_context is not None:
        # Factual recap of a stage that a full replan is being asked to
        # solve differently. Everything here already existed in the
        # failure_report / relevant_history produced when the replan was
        # scheduled -- this only reshapes it into text for the planner.
        system_prompt = (
            system_prompt
            + "\n\nRECOVERY CONTEXT (a previous local goal in this run "
            "could not be reached through repeated or locally modified "
            "execution, and required this full replan)\n"
            + f"Local goal that failed: {recovery_context.get('local_goal', '')}\n"
            + "Action(s) already tried and failed for this local goal:\n"
            + json.dumps(
                recovery_context.get("failed_actions", []),
                indent=2,
                ensure_ascii=False,
            )
            + "\nWhy a full replan was needed instead of repeating or "
            "modifying the same action: "
            + f"{recovery_context.get('reason_for_replan', '')}\n"
            + "Conditions that were violated or remained uncertain on the "
            "last attempt at that local goal:\n"
            + json.dumps(
                recovery_context.get("failed_conditions", [])
                + recovery_context.get("uncertain_conditions", []),
                indent=2,
                ensure_ascii=False,
            )
            + "\nLocal recovery strategies already exhausted on that stage "
            "before this replan (counts by type): "
            + json.dumps(
                recovery_context.get("local_strategies_already_tried", {}),
                ensure_ascii=False,
            )
            + f" across {recovery_context.get('total_attempts_on_stage', 0)} "
            "total attempts.\n"
            "This new plan replaces the failed approach above. Given why it "
            "failed, use your judgement on how to reach the same local "
            "goal -- a plan that is only a cosmetic variation of what has "
            "already been tried is unlikely to succeed for the same reasons."
        )
...
def execute_validator_step(
    ...
    print(
        f"[validator:{condition_kind}_{stage_id}] Scene context source: "
        "scene_object_list.json"
    )
...
def execute_goal_baseline_validator_step(
    ...
    print(
        f"[validator:goal_baseline_{stage_id}] Scene context source: "
        "scene_object_list.json"
    )
...
def execute_postcondition_validator_step(
    ...
    print(
        f"[validator:post_{stage_id}] Scene context source: "
        "scene_object_list.json"
    )
...
""".strip()
        ...
        # Evidence review is intentionally different from normal POST
        # validation: here the refreshed I_post scene graph is valid evidence.
        # Build a dedicated prompt instead of calling the normal POST wrapper,
        # which expects a prompt-file base_prompt and the static object list.
        system_prompt = f"""
...
def get_non_binary_ablation_output_dir(scenario_name: str, loop_timestamp: str) -> Path:
    return (
        Path("humanoids_results")
        / "ablation_study"
        / "validator"
        / "non_binary"
        / scenario_name
...
# run_with_terminal_log() must pick the log path -- and therefore a
# loop_timestamp -- before main() runs and computes its own. Sharing a single
# timestamp here keeps the terminal log, the run_summary.json this script
# saves under humanoids_results/, and the standard outputs/ artifacts all
# under the same identifying timestamp.
_SHARED_RUN_TIMESTAMP: str | None = None


def get_shared_run_timestamp() -> str:
    global _SHARED_RUN_TIMESTAMP
    if _SHARED_RUN_TIMESTAMP is None:
        _SHARED_RUN_TIMESTAMP = make_experiment_timestamp()
    return _SHARED_RUN_TIMESTAMP
...
def resolve_terminal_log_path(args: argparse.Namespace) -> Path:
    ...
    return (
        get_non_binary_ablation_output_dir(str(args.scenario), get_shared_run_timestamp())
        / "terminal_log.txt"
    )
...
def main() -> None:
    ...
    loop_timestamp = get_shared_run_timestamp()

    current_image = initial_image_path
    task_completed = False
    cycle_idx = 0

    # Recovery-latency instrumentation (repeat / modify / replan). A recovery
    # event is timed from the instant a cycle is confirmed to have a failed
    # attempt (all_stages_succeeded is False, about to look up failed_attempt)
    # until the PRE validation status of the first stage evaluated in the
    # cycle that resumes it is known again. This single span covers, for:
    #   - repeat/modify: the rest of the failing cycle (failure_report +
    #     interpret_failure + [modification_proposal for modify] +
    #     plan_recovery_evidence_based + schedule_recovery) plus the next
    #     cycle's scene_description + scene_description_full + PRE
    #     validation (vlm_planning/simultaneous_actions are skipped for a
    #     local reschedule).
    #   - replan: the same diagnostic segment, plus the next cycle's
    #     scene_description + scene_description_full + vlm_planning +
    #     simultaneous_actions + PRE validation.
    pending_recovery_timer_start: float | None = None
    pending_recovery_timer_decision: str | None = None
    pending_recovery_timer_cycle: int | None = None
    ...
    while not task_completed:
        ...
        try:
            ...
            local_reschedule = (
                isinstance(pending_schedule, dict)
                and pending_schedule.get("mode") == "local_reschedule"
            )

            if local_reschedule:
                # repeat/modify already carry their own stage and actions from
                # the previous failure (see schedule_recovery). Re-running
                # vlm_planning and simultaneous_actions here would only
                # produce a full new plan that gets discarded immediately
                # below, so both calls are skipped entirely.
                ...
                sequential_plan_artifact = None
                simultaneous_actions_artifact = None
                cycle_record["vlm_planning"] = None
                cycle_record["simultaneous_actions"] = None
                cycle_record["recovery_schedule"] = deepcopy(pending_schedule)
                full_summary["pending_recovery_schedule"] = None
                print(
                    f"[RECOVERY] Reusing stored stage/actions for a local "
                    f"reschedule with {len(stages)} pending stages "
                    "(vlm_planning and simultaneous_actions skipped)."
                )
            else:
                # A pending global_replan carries a compact, factual summary of
                # why local recovery (repeat/modify) was exhausted on the stage
                # that triggered it. This is the only place that summary is
                # ever turned into prompt text for an LLM: the planner would
                # otherwise regenerate a plan from the current scene alone,
                # with no way to know that a specific local goal already
                # failed and why.
                recovery_context = (
                    pending_schedule.get("replan_context")
                ...
                if recovery_context is not None:
                    print(
                        "[RECOVERY] vlm_planning prompt includes replan "
                        f"context for stage {recovery_context.get('stage_id')} "
                        f"(local_goal={recovery_context.get('local_goal')!r})."
                    )
                ...
                if not stages:
                    ...
                    # An empty planner output together with an empty scheduler output
                    # means that the current state requires no additional actions.
                    # This can happen when the task goal is already satisfied at the
                    # beginning of a cycle. Treat it as successful completion rather
                    # than as a scheduler error.
                    if sequential_plan_output == [] and scheduler_output == []:
                        cycle_record["outcome"] = "completed_no_actions_required"
                        cycle_record["end_image_path"] = str(
                        ...
                        raise ValueError(
                            "Scheduler returned no stages for a non-empty or "
                            "invalid sequential plan."
                        )

                    # Leave stages empty. The stage loop is skipped, and the
                    # completion block below keeps task_completed=True.

                if (
                    isinstance(pending_schedule, dict)
                    and pending_schedule.get("mode") == "global_replan"
                ):
                    cycle_record["recovery_schedule"] = deepcopy(pending_schedule)
                    full_summary["pending_recovery_schedule"] = None
            ...
            for stage_position, stage in enumerate(stages, start=1):
                ...
                if first_stage_in_cycle and pending_recovery_timer_start is not None:
                    t_ready = time.perf_counter()
                    recovery_duration = t_ready - pending_recovery_timer_start
                    recovery_timing_record = {
                        "recovery_index": len(full_summary["recovery_timings"]) + 1,
                        "decision": pending_recovery_timer_decision,
                        "triggered_in_cycle": pending_recovery_timer_cycle,
                        "resumed_in_cycle": cycle_idx,
                        "resumed_pre_status": pre_status,
                        "duration_seconds": recovery_duration,
                    }
                    full_summary["recovery_timings"].append(recovery_timing_record)
                    print(
                        f"[TIMING] Recovery #{recovery_timing_record['recovery_index']} "
                        f"({pending_recovery_timer_decision}) resolved in "
                        f"{recovery_duration:.3f}s (triggered in cycle "
                        f"{pending_recovery_timer_cycle}, resumed in cycle {cycle_idx} "
                        f"with PRE status={pre_status})."
                    )
                    pending_recovery_timer_start = None
                    pending_recovery_timer_decision = None
                    pending_recovery_timer_cycle = None
                first_stage_in_cycle = False
                ...
                if pre_status == "satisfied":
                    ...
                    # Normal POST validation intentionally runs before any
                    # enriched I_post scene reconstruction. The enriched graph
                    # is acquired only on demand for evidence or recovery.
                    ...
                    # A non-satisfied POST result enters the evidence/recovery
                    # path. Ensure that recovery has a fresh graph for I_post.
                    # When POST was uncertain, evidence gathering normally
                    # already produced it; a direct violation requires one
                    # fresh reconstruction here.
                    ...
                    # Transition analysis needs a graph corresponding to this
                    # attempt's actual I_pre. After a previous successful stage,
                    # the cycle-initial planning graph can be stale. Rebuild
                    # I_pre only when recovery is actually needed.
                    if post_status != "satisfied":
                        ...
                        attempt_record["pre"]["scene_description_full_path"] = (
                            pre_recovery_perception["paths"]["scene_graph"]
                        )
                        stage_record["pre_recovery_scene_perception"] = (
                            pre_recovery_perception
                        )
                    ...
                    # Promote a reconstructed I_post graph only when the stage
                    # entered evidence/recovery. On nominal success no enriched
                    # graph is created and no stale graph is relabelled as the
                    # authoritative current world state.
                    if post_status != "satisfied":
                        if post_perception is not None:
                            post_run_info_path = post_perception["paths"]["run_info"]
                            post_perception_seconds = post_perception[
                                "execution_time_seconds"
                            ]
                        else:
                            # The graph came from an evidence round.
                        ...
                        scene_description_full_artifact = {
                            ...
                            "paths": {
                                "artifact": attempt_record["post"].get(
                                    "scene_description_full_path"
                                ),
            ...
            if all_stages_succeeded and completed_stage_count == len(stages):
                # Every scheduled stage has been validated successfully by its
                # POST validator. The current image is therefore the validated
                # final state of this schedule. Do not start another planning
                # cycle and do not gate completion on the cycle-initial enriched
                # scene graph, which would be stale after successful actions.
                ...
                t_failure_detected = time.perf_counter()
                ...
                if not isinstance(failed_attempt, dict):
                    ...
                    remaining_stages = deepcopy(stages[failed_index + 1:])
                    failed_actions = (
                    ...
                    # Freeze actions for the stages still queued after the
                    # failed one. If a future cycle resumes this schedule as a
                    # local reschedule, vlm_planning/simultaneous_actions are
                    # skipped and no sequential_plan_artifact exists to look
                    # these actions up from later.
                    if sequential_plan_artifact is not None:
                        for remaining_stage in remaining_stages:
                            if not isinstance(remaining_stage.get("_actions"), list):
                                remaining_stage["_actions"] = extract_stage_actions(
                                    sequential_plan=sequential_plan_artifact["output"],
                                    step_ids=remaining_stage["Step_id"],
                                )
                    ...
                    if decision == "abort":
                        ...
                        if decision == "replan":
                            ...
                            # Compact, factual summary for the next cycle's
                            # vlm_planning prompt. Every field already exists
                            # elsewhere (failure_report, relevant_history,
                            # recovery_plan) -- nothing here is a new LLM
                            # summary, just a reshaping of existing data so
                            # the planner is not blind to this stage's history.
                            recovery_schedule["replan_context"] = {
                                "stage_id": failed_stage_id,
                                "local_goal": failed_stage.get("Local_goal", ""),
                                "reason_for_replan": recovery_plan.get("reason"),
                                "failed_actions": deepcopy(failed_actions),
                                "failed_conditions": failure_report.get(
                                    "failed_conditions", []
                                ),
                                "uncertain_conditions": failure_report.get(
                                    "uncertain_conditions", []
                                ),
                                "local_strategies_already_tried": deepcopy(
                                    relevant_history.get("strategy_counts", {})
                                ),
                                "total_attempts_on_stage": (
                                    relevant_history.get("retry_count", 0) + 1
                                ),
                            }
                        ...
                        pending_recovery_timer_start = t_failure_detected
                        pending_recovery_timer_decision = decision
                        pending_recovery_timer_cycle = cycle_idx

                    if args.interactive:
                        _interactive_pause_start = time.perf_counter()
                        input(
                            "\n[INTERACTIVE] Press Enter to continue to the "
                            "next cycle..."
                        )
                        _interactive_pause_seconds = (
                            time.perf_counter() - _interactive_pause_start
                        )
                        # A pending recovery timer was just armed above (or
                        # left untouched if decision == "abort"). Shift its
                        # start forward so this human pause is excluded from
                        # the measured recovery latency.
                        if pending_recovery_timer_start is not None:
                            pending_recovery_timer_start += _interactive_pause_seconds
    ...
    recovery_timings = full_summary["recovery_timings"]
    print("\n======================================================")
    print("RECOVERY LATENCY (hierarchical validator: repeat / modify / replan)")
    print(
        "Each entry: wall-clock time from the cycle confirming a failed "
        "attempt until the PRE validation status of the first stage of the "
        "resuming cycle is known -- i.e. failure_report + interpret_failure + "
        "[modification_proposal, for modify] + plan_recovery_evidence_based + "
        "schedule_recovery, plus the next cycle's scene_description + "
        "scene_description_full + PRE validation (repeat/modify) or the same "
        "plus vlm_planning + simultaneous_actions (replan). Any --interactive "
        "'Press Enter' pause between the decision and the next cycle is "
        "excluded."
    )
    print("------------------------------------------------------")
    if not recovery_timings:
        print("No repeat/modify/replan recovery was triggered during this run.")
    else:
        for entry in recovery_timings:
            print(
                f"  #{entry['recovery_index']:02d} | {entry['decision']:7s} | "
                f"cycle {entry['triggered_in_cycle']} -> cycle "
                f"{entry['resumed_in_cycle']} | {entry['duration_seconds']:.3f}s"
            )
        print("------------------------------------------------------")
        for decision_kind in ("repeat", "modify", "replan"):
            durations = [
                entry["duration_seconds"]
                for entry in recovery_timings
                if entry["decision"] == decision_kind
            ]
            if not durations:
                print(f"{decision_kind:7s}: none triggered")
                continue
            print(
                f"{decision_kind:7s}: n={len(durations)} | "
    ...
    if pending_recovery_timer_start is not None:
        print(
            "[WARN] The run ended while a "
            f"{pending_recovery_timer_decision} recovery triggered in cycle "
            f"{pending_recovery_timer_cycle} was still unresolved (max-attempts "
            "reached or the run errored out); it is not included above."
        )
    print("======================================================")

    non_binary_output_dir = get_non_binary_ablation_output_dir(args.scenario, loop_timestamp)
    non_binary_output_dir.mkdir(parents=True, exist_ok=True)
    run_summary_path = non_binary_output_dir / "run_summary.json"
    write_json(run_summary_path, full_summary)
    print(f"[OK][validation_loop] Run summary (incl. recovery_timings) saved to: {run_summary_path}")
