...
def build_scene_object_list_from_cycle(
    scenario: str,
    version: str,
    loop_timestamp: str,
    model: str,
    ...
    """
    Risolve automaticamente i path a partire da
    scenario/version/loop_timestamp/model/cycle_name.
    """
    settings = load_settings()
...
def build_parser() -> argparse.ArgumentParser:
    ...
    run_parser = subparsers.add_parser("run", help="Use legacy run-based layout.")
    run_parser.add_argument("--scenario", type=str, required=True)
    run_parser.add_argument("--version", type=str, required=True)
    run_parser.add_argument("--timestamp", type=str, required=True)
    run_parser.add_argument("--model", type=str, required=True)
    run_parser.add_argument("--run", type=str, default="run_001")

    cycle_parser = subparsers.add_parser("cycle", help="Use cycle-based validation-loop layout.")
    cycle_parser.add_argument("--scenario", type=str, required=True)
    cycle_parser.add_argument("--version", type=str, required=True)
    cycle_parser.add_argument("--loop-timestamp", type=str, required=True)
    cycle_parser.add_argument("--model", type=str, required=True)
    cycle_parser.add_argument("--cycle", type=str, required=True)
...
def main() -> None:
    ...
    if args.mode == "run":
        ...
        out_path = build_scene_object_list_from_cycle(
            scenario=args.scenario,
            version=args.version,
            loop_timestamp=args.loop_timestamp,
            model=args.model,
