...
def main() -> None:
    ...
    for scenario in scenarios:
        ...
        if not images:
            raise ValueError(f"Scenario '{scenario_id}' has no images")
                ...
                for prompt_file in prompt_files:
                    ...
                    for repeat_idx in range(1, args.repeats + 1):
                        ...
                        try:
                            ...
                            out_path = save_result(settings, output_name, result, scenario_id=scenario_id)
                            successful_runs += 1
                            print(f"[OK] {out_path}")
                            ...
                            output_name = (
                                f"{scenario_id}__{model_name}__{Path(prompt_file).stem}"
                                f"__{image_stem}__r{repeat_idx}__ERROR.json"
                            )

                            out_path = save_result(settings, output_name, error_payload, scenario_id=scenario_id)
