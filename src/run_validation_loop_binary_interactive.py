""" `run_validation_loop_binary_interactive.py` is a variant of `run_validation_loop.py`
(the offline, binary-validator pipeline) built for the Humanoids paper's
Table~\\ref{tab:validator_latency} comparison.

Two differences from `run_validation_loop.py`:

1. Instead of automatically consuming `--frames-dir` in chronological order, it asks
   from the terminal, before every POST validation, which image to use as I_post
   (same interaction pattern as `prompt_for_post_image` in the hierarchical
   validator's runner). Any image in `--frames-dir` is selectable, not only the
   naturally next one.
2. It measures "full replan latency" exactly as defined for the binary-validator
   baseline: the wall-clock time from the moment a validator call returns
   "non_matching" (PRE or POST) until the PRE validation status of the first stage
   of the next cycle is known again -- i.e. scene_description + scene_description_full
   + vlm_planning + simultaneous_actions + the next PRE validator call, which is
   exactly what a binary validator must redo on every mismatch since it never
   diagnoses *which* condition failed or *why*. Every full replan is timed
   individually with `time.perf_counter()`; the list and its summary statistics are
   printed at the end of the run and saved alongside the run summary.

The full terminal transcript and all run artifacts are saved under
`humanoids_results/ablation_study/validator/binary/<scenario>/<loop_timestamp>/`.
"""
...
from scene_enrichment_simulation import enrich_scene
from utils import (
    load_base_prompt,
...
class _TeeStream:
    """Writes to every wrapped stream so the terminal log also lands on disk."""
...
def get_binary_ablation_output_dir(settings, scenario_name: str, loop_timestamp: str) -> Path:
    return (
...
def prompt_for_next_image(frame_paths: list[str], frame_cursor: int) -> int:
    """
    Ask the user, in the terminal, which image inside --frames-dir to use as
    I_post for the current stage. Any image in frame_paths is selectable, not
    only the naturally next one. Returns the chosen index.
    """
    default_name = (
        Path(frame_paths[frame_cursor]).name
    ...
    for index, path in enumerate(frame_paths):
        marker = " (natural next)" if index == frame_cursor else ""
...
def main() -> None:
    ...
    run_output_dir = get_binary_ablation_output_dir(settings, args.scenario, loop_timestamp)
    run_output_dir.mkdir(parents=True, exist_ok=True)
    log_path = run_output_dir / "terminal_log.txt"
    log_file = open(log_path, "w", encoding="utf-8")
    original_stdout = sys.stdout
    sys.stdout = _TeeStream(original_stdout, log_file)

    try:
        _run(args, settings, loop_timestamp, run_output_dir)
    finally:
        sys.stdout = original_stdout
        log_file.close()
        print(f"[OK][validation_loop] Terminal log saved to: {log_path}")


def _run(args: argparse.Namespace, settings, loop_timestamp: str, run_output_dir: Path) -> None:
    ...
    # Replan-latency instrumentation. A "full replan" is timed from the
    # instant a validator call returns "non_matching" (PRE or POST) until the
    # PRE validation status of the first stage of the next cycle is known --
    # i.e. it covers exactly scene_description + scene_description_full +
    # vlm_planning + simultaneous_actions + the next PRE validator call, run
    # unconditionally by the binary validator on every mismatch.
    pending_replan_timer_start: float | None = None
    pending_replan_timer_reason: str | None = None
    pending_replan_timer_cycle: int | None = None
    ...
    print("\n======================================================")
    print("VALIDATION LOOP CONFIG (binary validator, interactive frames)")
    ...
    while not task_completed:
        ...
        try:
            ...
            for stage in stages:
                ...
                if first_stage_in_cycle and pending_replan_timer_start is not None:
                    t_ready = time.perf_counter()
                    replan_duration = t_ready - pending_replan_timer_start
                    replan_record = {
                        "replan_index": len(full_summary["replan_timings"]) + 1,
                        "triggered_in_cycle": pending_replan_timer_cycle,
                        "resumed_in_cycle": cycle_idx,
                        "trigger_reason": pending_replan_timer_reason,
                        "resumed_pre_result": pre_response["result"],
                        "duration_seconds": replan_duration,
                    }
                    full_summary["replan_timings"].append(replan_record)
                    print(
                        f"[TIMING] Full replan #{replan_record['replan_index']} resolved in "
                        f"{replan_duration:.3f}s (triggered in cycle {pending_replan_timer_cycle} "
                        f"by {pending_replan_timer_reason}, resumed in cycle {cycle_idx} with "
                        f"PRE result={pre_response['result']})."
                    )
                    pending_replan_timer_start = None
                    pending_replan_timer_reason = None
                    pending_replan_timer_cycle = None
                ...
                if pre_response["result"] == "non_matching":
                    ...
                    pending_replan_timer_start = time.perf_counter()
                    pending_replan_timer_reason = f"pre_non_matching_stage_{stage_id}"
                    pending_replan_timer_cycle = cycle_idx
                    break

                # --------------------------------------------------
                # DEPLOY PLACEHOLDER / OFFLINE NEXT IMAGE (asked interactively)
                # --------------------------------------------------
                chosen_index = prompt_for_next_image(frame_paths, frame_cursor)
                next_image = frame_paths[chosen_index]
                frame_cursor = chosen_index + 1
                ...
                if post_response["result"] == "non_matching":
                    ...
                    all_stages_succeeded = False
                    pending_replan_timer_start = time.perf_counter()
                    pending_replan_timer_reason = f"post_non_matching_stage_{stage_id}"
                    pending_replan_timer_cycle = cycle_idx
                    break
            ...
            print(f"\n[ERROR][validation_loop] Cycle {cycle_idx} failed: {exc}")
    ...
    replan_timings = full_summary["replan_timings"]
    print("\n======================================================")
    print("FULL REPLAN LATENCY (binary validator baseline)")
    print(
        "Each entry: wall-clock time from the validator call that returned "
        "'non_matching' until the PRE validation status of the first stage of "
        "the next cycle is known -- i.e. scene_description + "
        "scene_description_full + vlm_planning + simultaneous_actions + PRE "
        "validation, rerun unconditionally on every mismatch."
    )
    print("------------------------------------------------------")
    if not replan_timings:
        print("No full replan was triggered during this run.")
    else:
        for entry in replan_timings:
            print(
                f"  #{entry['replan_index']:02d} | cycle {entry['triggered_in_cycle']} -> "
                f"cycle {entry['resumed_in_cycle']} | trigger={entry['trigger_reason']} | "
                f"{entry['duration_seconds']:.3f}s"
            )
        durations = [entry["duration_seconds"] for entry in replan_timings]
        print("------------------------------------------------------")
        print(f"Replans measured: {len(durations)}")
        print(f"Mean:             {sum(durations) / len(durations):.3f}s")
        print(f"Min:              {min(durations):.3f}s")
        print(f"Max:              {max(durations):.3f}s")
        print(f"Total:            {sum(durations):.3f}s")
    if pending_replan_timer_start is not None:
        print(
            "[WARN] The run ended while a replan triggered in cycle "
            f"{pending_replan_timer_cycle} ({pending_replan_timer_reason}) was still "
            "unresolved (max-replans reached or the run errored out); it is not "
            "included above."
        )
    print("======================================================")
    ...
    print(f"[OK][validation_loop] Run summary (incl. replan_timings) saved to: {run_summary_path}")
