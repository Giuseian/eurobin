...
class CentauroCameraPush(Node):
    ...
    def __init__(self):
        ...
        if self.push_dagana not in (1, 2):
            raise ValueError(f'push_dagana={self.push_dagana} is invalid. Use 1 or 2.')
        ...
        if self.vector_norm((
            ...
            raise ValueError('The push direction cannot be the zero vector.')
    ...
    def set_arm_active(self, arm, active):
        ...
        self.get_logger().info(
            f'{"Activating" if active else "Deactivating"} {arm} task: {service_name}'
        )

        if not client.wait_for_service(timeout_sec=float(self.set_active_timeout)):
            raise RuntimeError(
                f'Service {service_name} is not available after '
        ...
        if response is None:
            raise RuntimeError(
                f'Call to {service_name} timed out after '
                f'{self.set_active_timeout:.1f} s.'
            )
        if not response.success:
            raise RuntimeError(
                f'{service_name} returned success=false: {response.message}'
            )
    ...
    def restore_full_pose_task_weights(self):
        ...
        if errors:
            raise RuntimeError(
                'Failed to fully restore the 6D arm task weights: ' + '; '.join(errors)
            )
    ...
    def lookup_tf_pose(self, target_frame, source_frame, timeout_s, label):
        ...
        raise RuntimeError(
            f'TF {target_frame} -> {source_frame} is not available after '
    ...
    def update_camera_pose_from_tf(self):
        ...
        self.get_logger().info(
            f'Camera pose composed in Cartesio world: '
    ...
    def camera_position_to_cartesian_world(self, camera_position):
        if self.camera_position is None or self.camera_orientation is None:
            raise RuntimeError('Camera pose unavailable: TF lookup has not been performed.')
    ...
    def parse_object_pose_file(self):
        ...
        if not path.is_file():
            raise FileNotFoundError(f'Object pose file not found: {path}')
        ...
        for line_number, raw_line in enumerate(path.read_text().splitlines(), start=1):
            ...
            if ':' in line:
                ...
                raise ValueError(f'{path}:{line_number}: use "key: values"')

            key = key.strip().lower().replace('.', '_')
            try:
                data[key] = tuple(float(value) for value in values.split())
            except ValueError as exc:
                raise ValueError(f'{path}:{line_number}: invalid numeric values') from exc
        ...
        for key, expected_length in required_lengths.items():
            if len(parsed[key]) != expected_length:
                raise ValueError(
                    f'{path}: "{key}" must contain {expected_length} values, '
                    f'but contains {len(parsed[key])}'
                )
    ...
    def push_target_components(self):
        ...
        self.get_logger().info(
            f'Object in computation frame: x={box_position_robot[0]:.6f}, '
            f'y={box_position_robot[1]:.6f}, z={box_position_robot[2]:.6f}, '
            f'yaw={box_yaw_robot:.6f} rad'
        )
        self.get_logger().info(
            f'Push contact in computation frame: x={contact_position_robot[0]:.6f}, '
            f'y={contact_position_robot[1]:.6f}, z={contact_position_robot[2]:.6f}'
        )
        self.get_logger().info(
            f'Push direction in computation frame: x={push_direction_robot[0]:.6f}, '
    ...
    def log_phase_targets(self, phases):
        self.get_logger().info('=== Computed push targets ===')
        self.get_logger().info(
            f'active push arm = {self.push_arm()}, inactive arm = {self.inactive_arm()}'
        )
    ...
    def execute(self):
        ...
        try:
            self.get_logger().info(
                f'Deactivating {self.inactive_arm()} to execute the push with {self.push_arm()}...'
            )
            ...
            self.get_logger().info('Reading object pose from file...')
            self.read_input_poses()

            phases = self.compute_all_phase_targets()
            if phases is None:
                raise RuntimeError('Unable to compute the absolute targets for the push phases.')

            self.get_logger().info('Configuring arm task weights...')
            ...
            if task_weights_configured and self.restore_orientation_weight_at_end:
                try:
                    self.get_logger().info('Restoring full 6D arm task weights...')
                    self.restore_full_pose_task_weights()
                    self.get_logger().info('6D arm task weights restored.')
                except Exception as exc:
                    self.get_logger().warn(f'Failed to restore 6D arm task weights: {exc}')
            if inactive_arm_disabled:
                try:
                    self.get_logger().info(
                        f'Reactivating {self.inactive_arm()} after the push...'
                    )
                    self.set_arm_active(self.inactive_arm(), True)
                    self.get_logger().info(f'{self.inactive_arm()} reactivated.')
                except Exception as exc:
                    self.get_logger().warn(
                        f'Failed to reactivate {self.inactive_arm()}: {exc}'
                    )
...
#### V1 - ORIGINAL WITH ITALIAN COMMENTS 
