...
python3 /home/user/eurobin/manipulation_code/centauro/push_box_xyz_direct.py --ros-args \
...
class CentauroDirectPush(Node):
    ...
    def __init__(self):
        ...
        if self.push_dagana not in (1, 2):
            raise ValueError(f'push_dagana={self.push_dagana} invalid. Use 1 or 2.')
        ...
        if self.vector_norm((
            ...
            raise ValueError('The push direction cannot be zero.')
    ...
    def set_arm_active(self, arm, active):
        ...
        self.get_logger().info(
            f'{"Activating" if active else "Deactivating"} task {arm}: {service_name}'
        )
    ...
    def restore_full_pose_task_weights(self):
        ...
        if errors:
            raise RuntimeError(
                'Incomplete restoration of 6D arm weights: ' + '; '.join(errors)
            )
    ...
    def parse_object_pose_file(self):
        ...
        for key, expected_length in required_lengths.items():
            if len(parsed[key]) != expected_length:
                raise ValueError(
                    f'{path}: "{key}" must have {expected_length} values, '
                    f'but has {len(parsed[key])}'
                )
    ...
    def log_phase_targets(self, phases):
        ...
        self.get_logger().info(
            f'active push arm = {self.push_arm()}, disabled arm = {self.inactive_arm()}'
        )
    ...
    def execute(self):
        ...
        try:
            self.get_logger().info(
                f'Deactivating {self.inactive_arm()} for push with {self.push_arm()}...'
            )
            self.set_arm_active(self.inactive_arm(), False)
            inactive_arm_disabled = True
            self.set_arm_active(self.push_arm(), True)

            self.get_logger().info('Reading poses from file...')
            self.read_input_poses()

            phases = self.compute_all_phase_targets()
            if phases is None:
                raise RuntimeError('Unable to compute the absolute phase targets.')
            ...
            if task_weights_configured and self.restore_orientation_weight_at_end:
                try:
                    ...
                    self.get_logger().info('6D arm weights restored.')
                except Exception as exc:
                    self.get_logger().warn(f'Failed to restore 6D arm weights: {exc}')
            if inactive_arm_disabled:
                try:
                    ...
                    self.get_logger().warn(
                        f'Reactivation of {self.inactive_arm()} failed: {exc}'
                    )
