#### V2 - ORIGINAL WITH ENGLISH COMMENTS
...
class CentauroCameraGrasp(Node):
    ...
    def restore_full_pose_task_weights(self):
        ...
        if errors:
            raise RuntimeError(
                'Failed to fully restore 6D arm task weights: ' + '; '.join(errors)
            )
    ...
    def lookup_tf_pose(self, target_frame, source_frame, timeout_s, label):
        ...
        self.get_logger().info(
            f'Searching TF {target_frame} -> {source_frame} for {label}...'
        )
        ...
        while rclpy.ok() and time.monotonic() < deadline:
            try:
                ...
                self.get_logger().info(
                    f'TF trovato for {label}: {target_frame} -> {source_frame}'
                )
                return pose
    ...
    def update_camera_pose_from_tf(self):
        ...
        self.get_logger().info(
            f'Camera composed in Cartesio world: '
    ...
    def box_target_components(self):
        ...
        self.get_logger().info(
            f'Grasp center in computation frame: x={grasp_position_robot[0]:.6f}, '
    ...
    def log_phase_targets(self, phases):
        ...
        self.get_logger().info(f'cartesian world TF = {self.cartesian_world_frame}')
        self.get_logger().info(f'cartesian pelvis TF = {self.cartesian_robot_base_frame}')
    ...
    def execute(self):
        ...
        try:
            ...
            self.get_logger().info('Reading object pose file...')
