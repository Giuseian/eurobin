...
"""
Centauro dual-arm grasp test from an object pose already expressed in target frame.

Run from this folder:

python3 grasp_centauro_bimanual.py --ros-args \
...
What it does:
    ...
    2. Uses that pose directly, without camera/world TF transformations.
...
Input pose file format:
    ...
    The position above is already expressed in base_frame. Object orientation is
    optional. If object.yaw or object.yaw_world is present, it is interpreted
    directly in base_frame. If yaw is omitted, yaw=0 in base_frame is used.

Most useful ROS parameters:
    object_pose_file:
        Text file containing the object pose in base_frame. Relative
...
class CentauroWorldGrasp(Node):

    def __init__(self):
        super().__init__('centauro_world_grasp')
