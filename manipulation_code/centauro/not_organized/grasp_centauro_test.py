...
What it does:
    ...
    2. Reads the camera pose from TF and transforms the object into the
       Centauro base frame.
    3. Uses a base-aligned object frame, ignoring object orientation.
...
Most useful ROS parameters:
    ...
    base_frame:
        Robot base frame used as target frame for the grasp. Default: pelvis.
    camera_frame:
        Camera frame in which object_pose_file is expressed. Default:
        D435_head_camera_gz_optical_frame.
    tf_lookup_timeout:
        Seconds to wait for the TF transform base_frame -> camera_frame.
    approach_mode:
...
class CentauroCameraGrasp(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('box_width', 0.4)
        ...
        self.declare_parameter('lift_z_phase4', 0.10)
    ...
    def update_camera_pose_from_tf(self):
        ...
        self.get_logger().info(
            f'Cerco TF {self.base_frame} -> {self.camera_frame} '
            f'per T_base_camera...'
        )
        ...
        while rclpy.ok() and time.monotonic() < deadline:
            try:
                ...
                self.camera_orientation = self.quat_normalize((
                    rotation.x,
                    rotation.y,
                    rotation.z,
                    rotation.w,
                ...
                self.get_logger().info(
                    f'TF camera trovato: {self.base_frame} -> {self.camera_frame}'
                )
                return
        ...
        raise RuntimeError(
            f'TF {self.base_frame} -> {self.camera_frame} non disponibile '
            f'dopo {timeout_s:.1f} s: {last_error}'
        )

    def camera_position_to_robot_base(self, camera_position):
    ...
    def read_input_poses(self):
        ...
        self.box_position = self.camera_position_to_robot_base(object_position_camera)
        self.box_orientation = (0.0, 0.0, 0.0, 1.0)

        self.log_pose('camera_in_base', self.camera_position, self.camera_orientation)
        self.get_logger().info(
            f'object_in_camera position: x={object_position_camera[0]:.6f}, '
            f'y={object_position_camera[1]:.6f}, z={object_position_camera[2]:.6f}'
        )
        self.log_pose('object_in_base', self.box_position, self.box_orientation)
    ...
    def box_target_components(self):
        ...
        self.get_logger().info(
            f'Oggetto in base Centauro: x={box_position_robot[0]:.6f}, '
