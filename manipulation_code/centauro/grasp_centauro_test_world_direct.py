...
"""
Centauro dual-arm grasp test from an object pose already expressed in world.

Run from this folder:

python3 /home/user/eurobin/manipulation_code/centauro/grasp_centauro_test_world_direct.py --ros-args \
...
What it does:
    1. Reads the object pose from a text file in the world frame.
    2. Uses that pose directly as the grasp reference, without TF transforms.
...
Input pose file format:
    ...
    The position above is expressed in base_frame/world. Object orientation is
    intentionally ignored; the grasp frame is aligned with base_frame.

Most useful ROS parameters:
    object_pose_file:
        Text file containing the object pose in the world frame. Relative
...
class CentauroWorldDirectGrasp(Node):

    def __init__(self):
        super().__init__('centauro_world_direct_grasp_test')
        ...
        if self.approach_mode not in ('yz', 'xz', 'xy'):
            raise ValueError(
                f'approach_mode="{self.approach_mode}" is invalid. Use: yz, xz, xy'
            )
    ...
    def restore_full_pose_task_weights(self):
        ...
        if errors:
            raise RuntimeError(
                'Incomplete restoration of 6D arm task weights: ' + '; '.join(errors)
            )
    ...
    def parse_object_pose_file(self):
        ...
        for key, expected_length in required_lengths.items():
            if len(parsed[key]) != expected_length:
                raise ValueError(
                    f'{path}: "{key}" must contain {expected_length} valori, '
    ...
    def read_input_poses(self):
        ...
        self.log_pose('object_in_world', self.box_position, self.box_orientation)
    ...
    def box_target_components(self):
        ...
        self.get_logger().info(
            f'Object in planning frame: x={box_position_robot[0]:.6f}, '
            f'y={box_position_robot[1]:.6f}, z={box_position_robot[2]:.6f}, '
            f'yaw={box_yaw_robot:.6f} rad'
        )
        self.get_logger().info(
            f'Grasp center in planning frame: x={grasp_position_robot[0]:.6f}, '
    ...
    def log_phase_targets(self, phases):
        self.get_logger().info('=== Computed grasp targets ===')
    ...
    def execute(self):
        ...
        try:
            self.get_logger().info('Reading world pose from file...')
            self.read_input_poses()

            phases = self.compute_all_phase_targets()
            if phases is None:
                raise RuntimeError('Unable to compute absolute phase targets.')
...
#         task_weights_configured = False
#         try:
#             self.get_logger().info('Lettura pose world da file...')
#             self.read_input_poses()
